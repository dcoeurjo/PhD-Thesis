...
Comme pour la droite,  une approche naturelle pour définir un cercle discret
...
Basé sur   cette analyse  des  paraboles discrètes,  \cite{laurethese}
...
La même année, \cite{Kim84b} ont proposé un algorithme permettant de
...
Un tel polygone convexe est aussi appelé {\it cellule de Voronoi
...
Le problème de séparation par arc se ramène donc au calcul
...
$[pq]$ est dans le premier octant. De plus nous supposons  qu'il existe deux entiers $a$,
...
nombres \citep{hardy} qui lie le nombre de points $I$ intérieur à un
...
de le arête joignant les pixels extrémités dans le cas d'un arc
...
donné   que la segmentation  en droites  utilisée n'est  pas optimal en
...
\citealt{P16,deberg}). Dans le cas où  les points sont issus d'une courbe discrète,
des algorithmes existent pour construire une telle enveloppe en $O(n)$
\citep{voss}.  
Le coût de  la  segmentation en droite discrète   étant
linéaire en le  nombre de points,  la construction de la courbe polygonale
...
sommets  inférieurs  de l'enveloppe convexe  au segment   $[(0,0),(b,a)]$  avec   $b>a>0$. Cet
...
$O(log(min(a,b))$.  À partir de ce processus, nous
...
correspondant aux pointillés d'appui. L'utilisation de l'algorithme de
\cite{voss} aurait introduit une complexité en  $O(max(a,b))$ pour le
segment considéré.
...
nous souhaitons avoir le domaine des centres possibles ou $acd$.
...
nous ne garantissons  un nombre minimal de  morceaux d'arcs. C'est pour
...
\begin{figure}[htbp]
  \begin{center}
    ...
    \caption{Exemples de segmentation en arcs de cercles~: $(a)$
...
un algorithme de segmentation d'une courbe discrète en morceaux d'arcs
de cercles, avec pour chaque morceau une description complète des
solutions.
